...
func connect(
	...
	endpoint, err := handler.Connect(logger, url, prompter, session, version, configuration, source)
	if err != nil {
...
func reconnect(
	...
	go func() {
		...
		endpoint, err := connect(logger, url, "", session, version, configuration, source)
