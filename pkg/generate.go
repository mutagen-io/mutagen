...
//go:generate protoc --plugin=./protoc-gen-go -I. --go_out=paths=source_relative,plugins=grpc:. session/configuration.proto session/scan_mode.proto session/selection.proto session/session.proto session/stage.proto session/state.proto session/watch.proto
