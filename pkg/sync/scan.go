...
func (s *scanner) directory(path string, directory *fs.Directory, metadata *fs.Metadata, parent *fs.Directory) (*Entry, error) {
	...
	for _, c := range directoryContents {
		...
		// ignore intermediate temporary files. It's only possible for scans to
		// see these files if they're generated by other sessions with
		// overlapping synchronization roots, but that's a very real
		// possibility. Note that these methods work regardless of Unicode
		// decomposition behavior on the filesystem.
		if fs.IsExecutabilityProbeFileName(name) {
			continue
		} else if fs.IsUnicodeProbeFileName(name) {
			continue
		} else if fs.IsTemporaryFileName(name) {
			continue
		}
