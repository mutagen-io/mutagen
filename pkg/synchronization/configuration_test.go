...
`
	testLegacyTOMLConfiguration = `[sync]
mode = "two-way-resolved"
maxEntryCount = 500
maxStagingFileSize = "1000 GB"
probeMode = "assume"
...
// TestLoadLegacyTOMLConfiguration tests loading a legacy TOML-based session
// configuration.
func TestLoadLegacyTOMLConfiguration(t *testing.T) {
	// Write a valid configuration to a temporary file and defer its cleanup.
	file, err := ioutil.TempFile("", "mutagen_configuration")
	if err != nil {
		t.Fatal("unable to create temporary file:", err)
	} else if _, err = file.Write([]byte(testLegacyTOMLConfiguration)); err != nil {
		t.Fatal("unable to write data to temporary file:", err)
	} else if err = file.Close(); err != nil {
		t.Fatal("unable to close temporary file:", err)
	}
	...
	// Attempt to load.
	tomlConfiguration := &LegacyTOMLConfiguration{}
	if err := encoding.LoadAndUnmarshalTOML(file.Name(), tomlConfiguration); err != nil {
		t.Error("configuration loading failed:", err)
	}
...
// TestLegacyGlobalConfigurationPath tests that LegacyGlobalConfigurationPath
// succeeds and returns a non-empty path.
func TestLegacyGlobalConfigurationPath(t *testing.T) {
	if path, err := LegacyGlobalConfigurationPath(); err != nil {
