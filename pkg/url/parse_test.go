...
func TestParseUnixForwardingTunnel(t *testing.T) {
	test := parseTestCase{
		raw:  "tunnel://tünnel:unix:/some/socket.sock",
		kind: Kind_Forwarding,
		expected: &URL{
			Kind:     Kind_Forwarding,
...
func TestParseTCP6ForwardingTunnel(t *testing.T) {
	test := parseTestCase{
		raw:  "tunnel://tünnel:tcp6:[::1]:5543",
		kind: Kind_Forwarding,
		expected: &URL{
			Kind:     Kind_Forwarding,
			Protocol: Protocol_Tunnel,
...
func TestParseTunnel(t *testing.T) {
	test := parseTestCase{
		raw:  "tunnel://tünnel/пат/to/the file",
		fail: false,
		expected: &URL{
			Protocol: Protocol_Tunnel,
			Host:     "tünnel",
...
func TestParseTunnelWithWindowsPath(t *testing.T) {
	test := parseTestCase{
		raw:  `tunnel://tünnel/C:\пат/to\the file`,
		fail: false,
		expected: &URL{
			Protocol: Protocol_Tunnel,
			Host:     "tünnel",
